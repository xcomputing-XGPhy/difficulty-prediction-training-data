...
class IQTreeError(Exception):
    ...
    If a subprocess.CalledProcessError is raised, this Exception captures
    lines starting with "ERROR" from the IQ-TREE output if available, or otherwise prints the full output.
    """
    def __init__(self, subprocess_exception: subprocess.CalledProcessError):
        error_information = []
        ...
        if error_information:
            ...
            error_details = "Check the following IQ-TREE log for more information on the error(s):\n"
            error_details += subprocess_exception.output
